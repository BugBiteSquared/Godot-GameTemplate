...
func _ready()->void:
  set_action_list()
  MenuEvent.connect("Controls",Callable(self,"show_controls"))
  #Localization
...
func set_action_list()->void:
  ...
  for Action in list:
    ...
    Name.text = Action
    Add.connect("pressed",Callable(self,"add_control").bind(Action))
...
func new_bind(Action, event)->void: #Adding bound InputEvent in the list
  ...
  Remove.connect("pressed",Callable(self,"remove_control").bind(Action, event, eventNode)) #Name, event, node
...
func get_InputEvent_name(event:InputEvent)->String:
  ...
  if event is InputEventKey:
    ...
    if Input.is_joy_known(event.device):
      text+= "ERROR"#str(Input.get_joy_button_string(event.button_index))
    else:
    ...
    var stick: = ''
    if Input.is_joy_known(event.device):
      stick = "ERROR"#str(Input.get_joy_axis_string(event.axis))
      text+= stick + " "
    else:
...
func remove_control(Bind:Array)->void:
  ...
  SettingsControls.ActionControls[Name].remove_at(index)

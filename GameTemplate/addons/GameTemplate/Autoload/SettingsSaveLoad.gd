...
#Save/ Load
#Call this method to trigger Settings saving - by default triggered checked closing options menu
...
func save_settings_JSON()->void:
  ...
  if not dir.dir_exists(CONFIG_DIR):
    dir.make_dir_recursive(CONFIG_DIR)
  var SettingsSaver:FileAccess = FileAccess.new()
  SettingsSaver.open(CONFIG_DIR + CONFIG_FILE_NAME + ".save", FileAccess.WRITE)
...
func load_settings_JSON()->bool:
  if Settings.HTML5: 										#need to confirm but for now means that HTML5 won't use the saving
    return	false
  #Json to Dictionary
  var SettingsLoader:FileAccess = FileAccess.new()
  if !SettingsLoader.file_exists(CONFIG_DIR + CONFIG_FILE_NAME + CONFIG_EXTENSION):
    return  false										#We don't have a save to load
  SettingsLoader.open(CONFIG_DIR + CONFIG_FILE_NAME + CONFIG_EXTENSION, FileAccess.READ)
  var test_json_conv = JSON.new()
  test_json_conv.parse(SettingsLoader.get_line())
